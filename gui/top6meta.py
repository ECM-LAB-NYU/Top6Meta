...
# Version: 1.0
#
# A Python framework for modeling architected materials and metastructures.
#
# Authors:
#   - Agyapal Singh [1]
...
transition_quality = 0.5                                     # for hybrid (transition quality range: 0.5-10 - translate to "Low", "Medium", "High")
...
y_rotation = 0
z_rotation = 0
resolution_points = 20                                      # resolution range: 50-1000
...
class GradingDialog(QDialog):
    ...
    def __init__(self, parent=None, global_var=None):
        ...
        if global_var == "TPMS" or global_var == "Spinodal":
            ...
            self.type_combo.addItems(["Linear"])
...
class SecondPage(QMainWindow):
    def __init__(self, parent=None):
        ...
        # info layout which is part of the right side (surface, vol fraction)
        ...
        info_layout.addWidget(tolerance_badge, alignment=Qt.AlignRight)
    ...
    def setup_tpms_config(self):
        ...
        points_layout, self.points_input = self.create_input_field("Points:", "20", QRegExpValidator(QRegExp("[0-9]*")))
    ...
    def popup_transition_options_hybrid(self):
        ...
        # Transition Quality, scale because slider doesn't support floats
        SCALE = 10
        ...
        self.transition_quality_slider.setMinimum(int(0.5 * SCALE))
        self.transition_quality_slider.setMaximum(int(10 * SCALE))
        self.transition_quality_slider.setValue(int(float(transition_quality) * SCALE))
        self.transition_quality_slider.setTickInterval(1 * SCALE)
        self.transition_quality_slider.setTickPosition(QSlider.TicksBelow)

        self.transition_quality_label = QLabel(
            f"Quality (k): {self.transition_quality_slider.value() / SCALE}"
        )

        label_container = QWidget()
        label_layout = QHBoxLayout()
        label_layout.addWidget(QLabel("Smooth"))
        label_layout.addStretch()
        label_layout.addWidget(QLabel("Gradual"))
        label_layout.addStretch()
        label_layout.addWidget(QLabel("Abrupt"))
        ...
        transition_quality_layout.addWidget(label_container)
        dialog_layout.addLayout(transition_quality_layout)

        self.transition_quality_slider.valueChanged.connect(
            lambda: [
                self.transition_quality_label.setText(
                    f"Quality (k): {self.transition_quality_slider.value() / SCALE}"
                ),
                self.update_transition_quality(
                    self.transition_quality_slider.value() / SCALE
                )
            ]
        )
    ...
    def popup_wrong_range_values(self, variable):
        ...
        if variable == "Sandwich":
            ...
            msg.setText("Resolution points should be between 20 and 200.")
    ...
    def check_range_resolution(self):
        ...
        if int(resolution_points) < 20 or int(resolution_points) > 200:
        ...
        if type == "TPMS":
            ...
            if grading == False:
                ...
                if grading_type == "Linear":
                    ...
                    print("Unsupported grading type for TPMS. Defaulting to Linear.")
                    params['FGType'] = 1
            ...
            if grading == False:
                ...
                if grading_type == "Linear":
                    ...
                    print("Unsupported grading type for Spinodal. Defaulting to Linear.")
                    params['FGType'] = 1
            ...
            if grading == False:
                ...
                if grading_type == "Linear":
                    ...
                    print("Unsupported grading type for Strut. Defaulting to Linear.")
                    params['FGType'] = 1
...
class AboutDialog(QDialog):
    def __init__(self, parent=None):
        ...
        label = QLabel(
            ...
            "Version 1.0<br><br>"
            "A Python framework for modeling architected materials "
